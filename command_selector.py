...
class FuzzySelector(ICommandSelector):
    ...
    def __build__(self, name, dict, force_rebuild):
        if force_rebuild:
            self.__occurence__(dict)
    ...
    def __frequency__(self, dict):
        for key in dict :
            ...
            for word in self.__words__:
                ...
                if word in words:
                    freq = float(words.count(word))/float(len(words)) * math.log(float(len(self.__tfidf__))/float(self.__words__[word] ))
